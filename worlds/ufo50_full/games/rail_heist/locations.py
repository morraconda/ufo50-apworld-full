...
# the three locations present on every level
CLEAR = "Clear"
ANGEL = "Angel Star"
DEVIL = "Devil Star"
...
# game_helpers.level_id -- offset = level * 10 + location-type slot. So A Simple Heist
...
def check_location_name(level: int, check_type: str) -> str:
    """Location name for one of a level's locations. The Clear location is just the level
...
class LocationInfo(NamedTuple):
    id_offset: int
    region_name: str
    level: int  # 1-indexed level number; 0 for the goal (Gift/Gold/Cherry) locations
...
def _build_location_table() -> dict[str, LocationInfo]:
    ...
    table["Gift"] = LocationInfo(997, "Levels", 0, "")
