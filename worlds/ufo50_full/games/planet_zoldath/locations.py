...
NPC_REWARDS: tuple[str, ...] = ("NPC Reward 1", "NPC Reward 2", "NPC Reward 3")
...
#    31..33    NPC Reward <n>  -- a trader that rolled the 1-in-3 gives an AP check
...
def _build_location_table() -> dict[str, LocationInfo]:
    ...
    for i, name in enumerate(NPC_REWARDS):
        table[name] = LocationInfo(31 + i, REGION)
...
def get_location_groups() -> dict[str, set[str]]:
    ...
    groups[f"{GAME_NAME} - NPC Rewards"] = {f"{GAME_NAME} - {name}" for name in NPC_REWARDS}
    return groups
