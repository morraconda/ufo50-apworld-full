...
EXPLOSIONS = f"{GAME_NAME} - Explosions"
GRUNT_DEFENCE = f"{GAME_NAME} - Grunt Defence"
...
# Promotion; from level GRUNT_LEVEL / EXPLOSIONS_LEVEL on you also need Grunt Defence /
# Explosions.
PROMO_LEVEL = 7
GRUNT_LEVEL = 21
EXPLOSIONS_LEVEL = 16
...
# PROMO_RANK on each milestone also needs the Promotion, and from rank GRUNT_RANK /
# EXPLOSIONS_RANK on it also needs Grunt Defence / Explosions.
PROMO_RANK = 50
GRUNT_RANK = 60
EXPLOSIONS_RANK = 70
...
def _grunt_defence(state: CollectionState, player: int) -> bool:
    return state.has(GRUNT_DEFENCE, player)


def _explosions(state: CollectionState, player: int) -> bool:
    return state.has(EXPLOSIONS, player)
...
def create_rules(world: "UFO50World", regions: dict[str, Region]) -> None:
    ...
    for n in range(1, NUM_LEVELS + 1):
        ...
        needs_grunt = n >= GRUNT_LEVEL
        needs_explosions = n >= EXPLOSIONS_LEVEL
        source.connect(
            regions[level_name(n)],
            rule=lambda state, lv=min(n, ENOUGH_TIME), promo=needs_promo,
            grunt=needs_grunt, expl=needs_explosions:
            _turn_time(state, player) >= lv
            and (not promo or _promoted(state, player))
            and (not grunt or _grunt_defence(state, player))
            and (not expl or _explosions(state, player)))

    # ranked ladder: Menu -> Rank 10 -> Rank 20 -> ... -> Rank 100 (open from the start,
    # Promotion-gated from rank 50, Grunt Defence-gated from rank 60, Explosions-gated
    # from rank 70)
    for i, r in enumerate(RANKS):
        source = regions["Menu"] if i == 0 else regions[rank_region(RANKS[i - 1])]
        needs_promo = r >= PROMO_RANK
        needs_grunt = r >= GRUNT_RANK
        needs_explosions = r >= EXPLOSIONS_RANK
        source.connect(
            regions[rank_region(r)],
            rule=lambda state, promo=needs_promo, grunt=needs_grunt, expl=needs_explosions:
            (not promo or _promoted(state, player))
            and (not grunt or _grunt_defence(state, player))
            and (not expl or _explosions(state, player)))
    ...
    set_rule(world.get_location(f"{GAME_NAME} - Cherry"),
             ...
             and _grunt_defence(state, player) and _explosions(state, player))
