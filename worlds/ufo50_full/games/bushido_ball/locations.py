...
# Bushido Ball is a 1v1 sword-ball tournament (o22_Game): you pick one of the 6
# fighters and beat the other 5. Each location is a match you win; the check fires
# the instant that match ends in your favour (o22_Game_Other_14), not on entering
# the next one. Any fighter can win, so all five rounds are sphere 1.

# id offset layout inside Bushido Ball's 1000-id block, via game_helpers.level_id
# (offset = level * 10 + slot):
#    10..50   Round <n>   (won tournament match n; level_id(n, 0))
...
def get_location_groups() -> dict[str, set[str]]:
    ...
    groups[f"{GAME_NAME} - Rounds"] = {f"{GAME_NAME} - Round {n}" for n in range(1, NUM_ROUNDS + 1)}
    return groups
