...
# Bushido Ball is a 1v1 tournament (o22_Game). You start with Chiyome unlocked;
# the other 5 fighters arrive from the multiworld and only owned fighters can be
# chosen at the select screen. Item id = 101 + vanilla portrait index (0..5).
# Nothing in logic needs a specific fighter.
...
# filler -- its filler is the do-nothing Encouragement (Bushido Ball is in `bad_filler_games`).
FILLER = "Encouragement"

# id offset layout inside Bushido Ball's 1000-id block:
#   101..106   the fighters (CHARACTERS order = vanilla char index 0..5)
