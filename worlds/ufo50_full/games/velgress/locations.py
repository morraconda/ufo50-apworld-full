...
#    N0        Level <N> Clear            (cleared area N -- reached its shop)
...
def _build_location_table() -> dict[str, LocationInfo]:
    ...
    for n in range(1, NUM_LEVELS + 1):
        table[f"Level {n} Clear"] = LocationInfo(level_id(n, 0), "Tower")
...
def get_location_groups() -> dict[str, set[str]]:
    ...
    groups[f"{GAME_NAME} - Clears"] = {f"{GAME_NAME} - Level {n} Clear" for n in range(1, NUM_LEVELS + 1)}
